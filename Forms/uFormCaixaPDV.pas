...
begin
  ...
  begin
    ...
    if not tb_query_venda.IsEmpty then // Verifica se a tabela não está vazia
  begin
    tb_query_venda.Edit; // Entra no modo de edição
    tb_query_venda.FieldByName('VALOR_TOTAL').AsFloat := TotalVenda; // Salva o total da venda no campo 'Total_Venda'
    tb_query_venda.Post; // Salva as alterações
  end;
end;
